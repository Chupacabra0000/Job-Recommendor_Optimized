...
from db import (
    ...
    replace_global_vacancies_for_area,   
    prune_global_vacancies_for_area,     
    list_all_global_vacancy_ids,     
...
def refresh_global_index(
    ...
    *,
    ...
    # 2.5) Upsert DB rows
    if need_full:
        replace_global_vacancies_for_area(int(cfg.area_id), rows)
    else:
        ...
        # prune old (best-effort)
        cutoff = (_utcnow() - timedelta(days=int(cfg.period_days) + 2)).isoformat().replace("+00:00", "Z")
        prune_global_vacancies_for_area(int(cfg.area_id), cutoff)
    ...
    if need_full or idx_existing is None or ids_existing is None:
        ...
        if np.any(mask):
            ...
            merged_ids = np.concatenate([
                ids_existing.astype(np.int64),
                add_ids.astype(np.int64)
            ])
    ...
    if need_full:
        try:
            all_vids = list_all_global_vacancy_ids()
            if all_vids:
                all_ids_i64 = np.asarray(
                    [_vid_to_int64(v) for v in all_vids],
                    dtype=np.int64
                )
                compact_store(MODEL_NAME, all_ids_i64)
        except Exception:
            # compaction failure should never break indexing
            pass
